...
// Порядок має значення так само, як у MODEL_QUESTIONS: береться ПЕРШИЙ збіг.
// Записи про тіла зі зрізом і про парні моделі стоять вище, бо містять
// корені «цилінд», «конус» і «prism_tri» (див. пастки в getSupportedUnfoldType).
// Кути й зсуви пораховані на геометрії моделей, а не підібрані на око.
const SECTION_PRESETS = [
  ...
  },
  {
    keys: ["зріз", "slice"],
    items: [
      { label: "Горизонтальний", axis: "y", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Уздовж зрізу", axis: "y", position: 0, tiltDeg: 35, azimuthDeg: 0 },
      { label: "Осьовий", axis: "x", position: 0, tiltDeg: 0, azimuthDeg: 0 },
    ],
  },
  {
    keys: ["куб", "cube", "паралелепіпед", "parallelepiped", "чотирикутна призма", "prism_square"],
    items: [
      { label: "Чотирикутник", axis: "y", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Діагональний", axis: "y", position: 0, tiltDeg: 45, azimuthDeg: 0 },
      { label: "П'ятикутник", axis: "y", position: 50, tiltDeg: 10, azimuthDeg: 30 },
      { label: "Шестикутник", axis: "y", position: 0, tiltDeg: 54.7, azimuthDeg: 45 },
      { label: "Трикутник", axis: "y", position: 55, tiltDeg: 54.7, azimuthDeg: 45 },
    ],
  },
  {
    keys: ["цилінд", "cylind"],
    items: [
      { label: "Коло", axis: "y", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Еліпс", axis: "y", position: 0, tiltDeg: 35, azimuthDeg: 0 },
      { label: "Осьовий", axis: "x", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Не через вісь", axis: "x", position: 50, tiltDeg: 0, azimuthDeg: 0 },
    ],
  },
  {
    keys: ["конус", "cone"],
    items: [
      { label: "Коло", axis: "y", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Еліпс", axis: "y", position: 0, tiltDeg: 30, azimuthDeg: 0 },
      { label: "Парабола", axis: "y", position: 0, tiltDeg: 63.4, azimuthDeg: 0 },
      { label: "Через вершину", axis: "x", position: 0, tiltDeg: 0, azimuthDeg: 0 },
    ],
  },
  {
    keys: ["пірамід", "piramide", "pyramid", "тетраедр", "tetrahedron"],
    items: [
      { label: "Паралельно основі", axis: "y", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Ближче до вершини", axis: "y", position: 45, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Через висоту", axis: "x", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Похилий", axis: "y", position: 0, tiltDeg: 30, azimuthDeg: 60 },
    ],
  },
  {
    keys: ["призм", "prism"],
    items: [
      { label: "Перпендикулярно осі", axis: "y", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Похилий", axis: "y", position: 0, tiltDeg: 30, azimuthDeg: 0 },
      { label: "Уздовж осі", axis: "x", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Повернутий", axis: "y", position: 0, tiltDeg: 30, azimuthDeg: 90 },
    ],
  },
  {
    keys: ["сфер", "sphere"],
    items: [
      { label: "Велике коло", axis: "y", position: 0, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Мале коло", axis: "y", position: 55, tiltDeg: 0, azimuthDeg: 0 },
      { label: "Похиле", axis: "y", position: 30, tiltDeg: 40, azimuthDeg: 0 },
    ],
  },
];

/** Пресети для активної моделі; порожній масив, якщо збігу немає. */
function pickSectionPresets(asset) {
  const hay = `${asset?.title ?? ""} ${asset?.filePath ?? ""}`.toLowerCase();
  const match = SECTION_PRESETS.find((entry) => entry.keys.some((key) => hay.includes(key)));

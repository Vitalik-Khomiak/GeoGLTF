...
// Пресети перерізів (SECTION_PRESETS у app.js) — стартові позиції площини для
// кожної фігури бібліотеки. Числа нижче пораховані на реальній геометрії, як
// і CONTROL/DEGENERATE вище, а не підібрані на око: перевірка тут закріплює
// їх до появи самої таблиці (крок 3), тому має бути зеленою вже зараз.
console.log("Пресети перерізів");
const PRESETS = [
  ["cube.glb",      "Чотирикутник", { axis: "y", position: 0,  tilt: 0,    azimuth: 0  }, [4],  4.00, 8.00],
  ["cube.glb",      "Шестикутник",  { axis: "y", position: 0,  tilt: 54.7, azimuth: 45 }, [6],  5.20, 8.49],
  ["cube.glb",      "Трикутник",    { axis: "y", position: 55, tilt: 54.7, azimuth: 45 }, [3],  1.58, 5.73],
  ["cube.glb",      "П'ятикутник",  { axis: "y", position: 50, tilt: 10,   azimuth: 30 }, [5],  3.52, 7.47],
  // Брифом заявлено 8 вершин, але діагональний переріз куба з ребром 2 —
  // прямокутник 2×2√2 (4 вершини): зайві точки лежать на довгих сторонах
  // і прибираються порогом mergeCollinearLoopPoints. S/P із брифу коректні.
  ["cube.glb",      "Діагональний", { axis: "y", position: 0,  tilt: 45,   azimuth: 0  }, [4],  5.66, 9.66],
  ["Cylynder.glb",  "Коло",         { axis: "y", position: 0,  tilt: 0,    azimuth: 0  }, null, 3.06, 6.24],
  ["Cylynder.glb",  "Еліпс",        { axis: "y", position: 0,  tilt: 35,   azimuth: 0  }, null, 3.74, 6.95],
  ["Cylynder.glb",  "Осьовий",      { axis: "x", position: 0,  tilt: 0,    azimuth: 0  }, [4],  4.00, 8.00],
  ["cone.glb",      "Коло",         { axis: "y", position: 0,  tilt: 0,    azimuth: 0  }, null, 0.78, 3.14],
  ["cone.glb",      "Еліпс",        { axis: "y", position: 0,  tilt: 30,   azimuth: 0  }, null, 1.03, 3.62],
  ["cone.glb",      "Парабола",     { axis: "y", position: 0,  tilt: 63.4, azimuth: 0  }, null, 1.93, 5.66],
  ["cone.glb",      "Через вершину",{ axis: "x", position: 0,  tilt: 0,    azimuth: 0  }, null, 2.00, 6.47],
];
for (const [file, label, opts, vertices, area, perimeter] of PRESETS) {
  const r = sectionOf(file, opts);
  check(`${file} / ${label}: один контур`, r.loops === 1, `${r.loops}`);
  if (vertices) check(`${file} / ${label}: вершин ${vertices[0]}`, r.vertices[0] === vertices[0], `${r.vertices[0]}`);
  check(`${file} / ${label}: S ≈ ${area.toFixed(2)}`, Math.abs(r.area - area) < 0.01, r.area.toFixed(2));
  check(`${file} / ${label}: P ≈ ${perimeter.toFixed(2)}`, Math.abs(r.perimeter - perimeter) < 0.01, r.perimeter.toFixed(2));
}

// SECTION_PRESETS і pickSectionPresets — не чисті функції в списку NAMES вище
// (перша — масив об'єктів, друга — функція, що читає її), тож вирізаються
// окремою фабрикою за зразком getSupportedUnfoldType у tools/test-unfold.mjs:126.
// Перевірка навмисно повторює саме той порядок записів, що й у файлі: якщо
// вужчі записи (парні моделі, тіла зі зрізом) опиняться нижче за загальні —
// «Два подібні конуси» чи «Трикутна призма зі зрізом» отримають чужий пресет.
console.log("Маршрутизація пресетів за назвою моделі");
...
const ROUTING = [
  ...
  ["Трикутна призма зі зрізом", "./assets/models/prism_tri_slice.glb", "Горизонтальний"],
  ["Куб", "./assets/models/cube.glb", "Чотирикутник"],
  ["Циліндр", "./assets/models/Cylynder.glb", "Коло"],
  ["Конус", "./assets/models/cone.glb", "Коло"],
  ["Правильна трикутна призма", "./assets/models/prism_tri.glb", "Перпендикулярно осі"],
  ["Сфера", "./assets/models/sphere.glb", "Велике коло"],
];
for (const [title, filePath, firstLabel] of ROUTING) {
  const items = pickSectionPresets({ title, filePath });
  check(`«${title}» -> «${firstLabel}»`, items[0]?.label === firstLabel, items[0]?.label ?? "нічого");
}
